"""Lint ADFS ROM disassembly driver scripts and documentation.
...
def load_address_ranges(json_filepath):
    ...
    end_addr = meta.get('end_addr', load_addr + 0x4000)
...
def lint(version_dirpath, version):
    ...
    script_filename = f"disasm_{pfx.replace('-', '_')}_{version.replace('.', '').lower()}.py"
    ...
    if not json_filepath.exists():
        ...
        print("Run 'acorn-adfs-disasm-tool disassemble' first.", file=sys.stderr)
        return 1
    ...
    for ann in annotations:
        if ann["kind"] == "subroutine":
            ...
            if addr in seen_labels:
                errors.append(
                    f"  line {ann['line_number']}: "
                    f"duplicate label(0x{addr:04X}) "
                    f"— first at line {seen_labels[addr]}"
                )
            else:
                seen_labels[addr] = ann["line_number"]

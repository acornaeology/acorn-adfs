"""CLI entry point for acorn-adfs-disasm-tool."""
...
def rom_prefix(version_dirpath):
    """Return the ROM prefix for a version directory."""
...
def cmd_disassemble(args):
    """Run the disassembly for a given version."""
    version_dirpath = get_version_dirpath(args.version)
    prefix = rom_prefix(version_dirpath)
    script_filename = f"disasm_{prefix.replace('-', '_')}_{args.version.replace('.', '').lower()}.py"
    ...
    env = os.environ.copy()
    env["ACORN_ADFS_ROM"] = str(version_dirpath / "rom" / f"{prefix}-{args.version}.rom")
    env["ACORN_ADFS_OUTPUT"] = str(version_dirpath / "output")
...
def cmd_backfill(args):
    """Propagate annotations from one version to another."""
    from disasm_tools.backfill import backfill

    source_dirpath = get_version_dirpath(args.source)
    target_dirpath = get_version_dirpath(args.target)
    sys.exit(backfill(source_dirpath, args.source,
                      target_dirpath, args.target,
...
def main():
    parser = argparse.ArgumentParser(
        ...
        description="Acorn ADFS ROM disassembly tools",
    )
    subparsers = parser.add_subparsers(dest="command", required=True)

    dis_parser = subparsers.add_parser(
        "disassemble", help="Run disassembly for a version"
    )
    dis_parser.add_argument("version", help="Version (e.g. 1.30)")
    dis_parser.set_defaults(func=cmd_disassemble)

    verify_parser = subparsers.add_parser(
        "verify", help="Verify disassembly round-trips to original ROM"
    )
    verify_parser.add_argument("version", help="Version (e.g. 1.30)")
    verify_parser.set_defaults(func=cmd_verify)

    lint_parser = subparsers.add_parser(
        "lint", help="Validate annotation addresses in driver script"
    )
    lint_parser.add_argument("version", help="Version (e.g. 1.30)")
    lint_parser.set_defaults(func=cmd_lint)

    compare_parser = subparsers.add_parser(
        "compare", help="Compare two ROM versions"
    )
    compare_parser.add_argument("version_a", help="First version (e.g. 1.30)")
    compare_parser.add_argument("version_b", help="Second version")
    compare_parser.set_defaults(func=cmd_compare)

    extract_parser = subparsers.add_parser(
        "extract", help="Extract assembly section by address or label"
    )
    extract_parser.add_argument("version", help="Version (e.g. 1.30)")
    ...
    audit_parser.add_argument("version", help="Version (e.g. 1.30)")
    ...
    insert_parser.add_argument("version", help="Version (e.g. 1.30)")
    ...
    cfg_parser.add_argument("version", help="Version (e.g. 1.30)")
    ...
    context_parser.add_argument("version", help="Version (e.g. 1.30)")
    ...
    labels_parser.add_argument("version", help="Version (e.g. 1.30)")
    ...
    check_parser.add_argument("version", help="Version (e.g. 1.30)")
    ...
    rename_parser.add_argument("version", help="Version (e.g. 1.30)")
    ...
    backfill_parser = subparsers.add_parser(
        "backfill", help="Propagate annotations between versions"
    )
    backfill_parser.add_argument("source", help="Source version (e.g. 1.30)")
    backfill_parser.add_argument("target", help="Target version")
